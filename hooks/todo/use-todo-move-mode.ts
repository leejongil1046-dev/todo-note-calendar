...
export function useTodoMoveMode({
  ...
  const moveTodoByStep = useCallback(
    (todoId: number, direction: "up" | "down") => {
      setTodos((prevTodos) => {
        const currentIndex = prevTodos.findIndex(
        ...
        if (nextIndex < 0 || nextIndex >= prevTodos.length) return prevTodos;
        ...
        if (dateString) {
          onTodoSummaryChanged(
            dateString,
            buildTodoSummaryFromTodos(nextTodos),
          );
        }
    ...
    [setTodos, dateString, onTodoSummaryChanged],
  );
